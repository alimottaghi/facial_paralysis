...
def main():
    ...
    if save_out_video:
        ...
        videoWriter = cv2.VideoWriter(
            os.path.join(args.out_video_root,
                         f'pose_{os.path.basename(args.video_path)}'), fourcc,
            fps, size)
    ...
    cnt = 0
    pose_data = {}
    for cnt, cur_frame in enumerate(mmcv.track_iter_progress(video)):
        ...
        vis_frame = vis_pose_result(
            pose_model,
            0*np.zeros_like(cur_frame),  # cur_frame,
            pose_results,
            radius=args.radius,
        ...
        if save_out_video:
            ...
            pose_data[str(cnt)] = pose_results
    ...
    if save_out_video:
        ...
        np.save(
            os.path.join(args.out_video_root, 
            f'pose_{os.path.splitext(os.path.basename(args.video_path))[0]}.npy'), 
            pose_data)
    if args.show:
